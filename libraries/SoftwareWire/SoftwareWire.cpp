...
SoftwareWire::SoftwareWire(uint8_t sdaPin, uint8_t sclPin, boolean pullups, boolean detectClockStretch)
{
  ...
  _sdaPortReg  = (uint8_t *)portOutputRegister(port);
  _sdaDirReg   = (uint8_t *)portModeRegister(port);
  _sdaPinReg   = (uint8_t *)portInputRegister(port);      // PinReg is the input register, not the Arduino pin.

  port = digitalPinToPort(_sclPin);
  _sclBitMask  = digitalPinToBitMask(_sclPin);
  _sclPortReg  = (uint8_t *)portOutputRegister(port);
  _sclDirReg   = (uint8_t *)portModeRegister(port);
  _sclPinReg   = (uint8_t *)portInputRegister(port);
}
